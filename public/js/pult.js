...
var saldoComplete = -1
var timerId
if (localStorage.getItem("saldoRuning") == 'false' && parseInt(localStorage.getItem("saldoRuningDay")) != (new Date()).getDate()) {
    saldoTarget()
    saldoMessage()
}
    ...
    for (osszetevo of state.osszetevok) {
        if (tBeszar[osszetevo.termek_id]) {
            tBeszar[osszetevo.termek_id] +=
                ...
                aBeszar[osszetevo.alapanyag_id];
        } else {
            tBeszar[osszetevo.termek_id] =
                osszetevo.felhasznaltmennyiseg *
                aBeszar[osszetevo.alapanyag_id];
...
function renderProducts() {
    ...
    function renderPultFirst() {
        ...
        for (let tabIndex = 0; tabIndex <= allTabs; tabIndex++) {
            ...
            <a class="nav-link font-weight-bolder border border-dark p-1 ${tabIndex == 0 ? "active" : ""
                }" href="#menu${tabIndex} " style="font-size: 135%"><kbd>- ${tabIndex + 1} -</kbd></a>
        </li>`;
        }
        ...
        if (kosarbolVisszatoltott) {
            ...
            $(".keyboard").on("click", function () {
                ...
                if (inputKey == "") {
                    ...
                    kosarMegnevezes = kosarMegnevezes.substring(0, kosarMegnevezes.length - 1)
                } else { kosarMegnevezes += inputKey };
                //VERSION-2:
                document.querySelector("#kosarMegnevezesId").value =
                    kosarMegnevezes;
            });
        ...
        if (foundKosar) {
            ...
            $(".zzzzz").click(function () {
                if (state.kosarak[this.id].length == 0) {
                    /* alert("Ezt most törlöm mert üres kosár!"); *///VERSION-2:
                    state.kosarak.splice(this.id, 1);
                    state.kosarNevek.splice(this.id, 1);
                    kosarbolVisszatoltott = false;
                    foundKosar = state.kosarak.length > 0 ? true : false;
...
function visszajaro() {
    ...
    document.getElementById('backReturnHead').innerHTML = `Fizetendő:     ${mindossz},- Ft`
    for (cimlet of cimletek) {
        if (mindosszesenTransaction < cimlet) {
            ...
            <td class="text-right"><h3><span class="font-weight-bold">${ezresCsoportosit(cimlet - mindosszesenTransaction)},-</span> Ft</td>
            </tr>
            `;
...
function trKp2() {
    if (mindosszesenTransaction == 0) {
        ...
        async function kp2query() {
            ...
            setTimeout(() => { $("#responseKP2DaySUM").modal("hide") }, 4000)
        }
    } else {
        let trFizetesMod = "m";
        trNumber = createTrNumber();
...
function createTranactionData(
    ...
    async function updateMySQL() {
        ...
        if (kosarbolVisszatoltott) {
            pultos =
                state.kosarNevek[kosarbolVisszatoltottId]
...
$(".kpKivet").click(function () {
    ...
    $(".keyboard").on("click", function () {
        ...
        if (inputKey == "") {
            console.log('torleesgomb')
            kivetMegnevezes = kivetMegnevezes.substring(0, kivetMegnevezes.length - 1)
        } else { kivetMegnevezes += inputKey };
...
function ezresCsoportosit(ezresCsoportNormal) {
    ezresCsoport = (ezresCsoportNormal).toLocaleString(
        "hu-HU",
        {
            maximumFractionDigits: 0,
        }
...
function saldoMessage() {
    ...
    let timerId = setTimeout(saldoCalculation, saldoReactDatumEzred - saldoDatumEzred)
...
async function saldoCalculation() {
    ...
    saldoComplete = (saldo - responseKP2DaySUM - responseCARDDaySUM).toLocaleString("hu-HU", {
        maximumFractionDigits: 0,
    })
...
$("#other").on("click", () => {
    ...
    $("#sendData").on("click", () => {
        ...
        data.push({ dataType: "Kávégép számlálóállása", dataValue: document.getElementById("adat1").value })
        if (document.getElementById("adat2").value == '') {
            document.getElementById("adat2").value = state.otherlist[1].dataValue
        }
        data.push({ dataType: "Játékgépek számlálóállása", dataValue: document.getElementById("adat2").value })
        if (document.getElementById("adat3").value == '') {
            document.getElementById("adat3").value = state.otherlist[2].dataValue
        }
        data.push({ dataType: "Csocsó számláló", dataValue: document.getElementById("adat3").value })
        if (document.getElementById("adat4").value == '') {
            document.getElementById("adat4").value = state.otherlist[3].dataValue
        }
        data.push({ dataType: "Biliárd", dataValue: document.getElementById("adat4").value })
        data.push({ dataType: "Dátum", dataValue: datumNow })

        data.push({ dataType: "Kávégép előző számlálóállása", dataValue: state.otherlist[0].dataValue })
        data.push({ dataType: "Játékgépek előző számlálóállása", dataValue: state.otherlist[1].dataValue })
        data.push({ dataType: "Csocsó előző számláló", dataValue: state.otherlist[2].dataValue })
        data.push({ dataType: "Biliárd előző", dataValue: state.otherlist[3].dataValue })
        data.push({ dataType: "Dátum előző", dataValue: state.otherlist[4].dataValue })
        otherTarol()
