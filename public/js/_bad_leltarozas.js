...
function screenChange() {
    ...
    $("#leltarFeldolgozas").on("click", function () {
        ...
        */
        ...
        for (alapanyag of state.alapanyagok) {
            ...
            //VERSION-2:
            //INFO: ez a state[].keszletsum
            /* state.alapanyagok[alapanyagIndex].keszletsum = Math.round(nowKeszletDB * state.alapanyagok[alapanyagIndex].kiszereles) + nowKeszletPULT */
            //VERSION-2:
        ...
        function leltarTarol() {
            var alapanyagIndex = 0
            var validKeszlet = 0
            var afterLELTARsumBDdata = []
            //VERSION-2:
            getAfterData()
            ...
            //VERSION-2:
            //getdata()
            //VERSION-2:for (alapanyag of state.alapanyagok) {
            //VERSION-2:state.pultbanVan[state.alapanyagok[alapanyagIndex].id] = state.pultbanVan[state.alapanyagok[alapanyagIndex].id] == null ? 0 : state.pultbanVan[state.alapanyagok[alapanyagIndex].id]
            //console.log(state.alapanyagok[alapanyagIndex].id)
            //console.log(localLeltarozottKeszlet[alapanyagIndex])
            //console.log(state.pultbanVan[state.alapanyagok[alapanyagIndex].id])

            //VERSION-2:
            /* if (dataLocal[alapanyagIndex].keszlet == 0) {
                validKeszlet = alapanyag.keszletsum
            } else {
                validKeszlet = dataLocal[alapanyagIndex].keszlet * alapanyag.kiszereles + state.pultbanVan[state.alapanyagok[alapanyagIndex].id] * alapanyag.kiszereles
            } */
            //VERSION-2:

            /* afterLELTARsumBDdata.push(dataLocal[alapanyagIndex].keszlet * alapanyag.kiszereles + state.pultbanVan[state.alapanyagok[alapanyagIndex].id] * alapanyag.kiszereles) */
            //VERSION-2:afterLELTARsumBDdata.push(validKeszlet)
            /* console.log('---------------------------------------------')
            console.log('alapanyag.kiszereles')
            console.log(alapanyag.kiszereles)
            console.log('alapanyag.keszletsum')
            console.log(alapanyag.keszletsum)
            console.log('dataLocal[alapanyagIndex].keszlet * alapanyag.kiszereles')
            console.log(dataLocal[alapanyagIndex].keszlet * alapanyag.kiszereles)
            console.log('state.pultbanVan[state.alapanyagok[alapanyagIndex].id] * alapanyag.kiszereles')
            console.log(state.pultbanVan[state.alapanyagok[alapanyagIndex].id] * alapanyag.kiszereles)
            console.log('dataLocal')
            console.log(dataLocal)
            console.log('---------------------------------------------') */
            ...
            //VERSION-2:localLeltarozottKeszlet[alapanyagIndex] += state.pultbanVan[state.alapanyagok[alapanyagIndex].id]

            //console.log('---------------------------------------------')

            //VERSION-2:alapanyagIndex++
            //VERSION-2:}

            //VERSION-2:
            /* localStorage.setItem("afterLELTARsumBD", JSON.stringify(afterLELTARsumBDdata))
            ...
            console.log(localStorage.getItem("beforeLELTARsumBD"))
            console.log(localStorage.getItem("afterLELTARsumBD")) */
            //VERSION-2:
            ...
            async function updateLeltar() {
                await fetch("/updateleltar", {
                    ...
                    body: JSON.stringify({ data: dataLocal }),
                });
    ...
    $("#leltarozottDB").on("click", function () {
        document.getElementById("leltarozottDB").value = 0;
    });
...
function leltarFelvitel() {
    ...
    $("#leptetElore").on("click", function () {
        v_leltarozottDB = parseInt(
            document.getElementById("leltarozottDB").value
        );
...
function renderAlapanyag() {
    ...
    /* nowKeszletPULT = Math.round(state.alapanyagok[alapanyagIndex].keszletsum - Math.round(nowKeszletDB * state.alapanyagok[alapanyagIndex].kiszereles) * 100) / 100 */
    ...
    //VERSION-2:
    //localLeltarDB[alapanyagIndex] = nowKeszletDB
    localLeltarPult[alapanyagIndex] = nowKeszletPULT
    //localLeltarozottKeszlet[alapanyagIndex] = nowKeszletDB
    /* state.alapanyagok[alapanyagIndex].keszletsum = Math.round(localLeltarDB[alapanyagIndex]
        * state.alapanyagok[alapanyagIndex].kiszereles) + localLeltarPult[alapanyagIndex] */
    //VERSION-2:
    document.getElementById("alapanyagNev").innerHTML =
        state.alapanyagok[alapanyagIndex].nev;

    //VERSION-2:INFO:VERSION-2:
    document.getElementById("leltarozottDB").value = parseInt(
        nowKeszletDB
    );
    document.getElementById("leltarozottPULT").value = parseFloat(
        nowKeszletPULT
        /* document.getElementById("leltarozottDB").value = parseInt(
            localLeltarDB[alapanyagIndex]
        );
        document.getElementById("leltarozottPULT").value = parseFloat(
            localLeltarPult[alapanyagIndex] */
    );
...
function saveLocalStorageLeltarData(index, darab, rekesz, pult) {
    ...
    sumDB = (rekesz * state.alapanyagok[alapanyagIndex].gyujto) + darab + state.pultbanVan[id]

    itemKeszletSum = Math.round((sumDB * state.alapanyagok[alapanyagIndex].kiszereles + pult) * 100) / 100
    //VERSION-2:
    /* var afterLELTARsumBDdata = []
    afterLELTARsumBDdata.push(localLeltarozottKeszlet[alapanyagIndex])
    localStorage.setItem("afterLELTARsumBD", JSON.stringify(afterLELTARsumBDdata)) */
    //VERSION-2:
